...
def create_mnistm(X, label):
    ...
    labels = np.zeros((X.shape[0], 10), np.uint8)
...
train, train_label = create_mnistm(mnist.train.images, mnist.train.labels)
print('Building test set...')
test, test_label = create_mnistm(mnist.test.images, mnist.test.labels)

# Save dataset as pickle
with open('mnistm_data.pkl', 'wb') as f:
    pkl.dump(
        {'train': train, 'train_label': train_label, 'test_label': test_label, 'test': test},
